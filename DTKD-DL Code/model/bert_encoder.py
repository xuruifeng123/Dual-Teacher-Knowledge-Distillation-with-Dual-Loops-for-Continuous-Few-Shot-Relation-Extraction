    ...
    def forward(self, inputs):
        ...
        if self.pattern == 'standard':
            ...
            for i in range(len(e11)):
                instance_output = torch.index_select(tokens_output, 0, torch.tensor(i).to(self.config.device))
                instance_output = torch.index_select(instance_output, 1, torch.tensor([e11[i], e21[i]]).to(self.config.device))
...
class desc_encoder(base_model):
    def __init__(self,config):
        super(desc_encoder, self).__init__()
        self.config = config
        self.encoder_1 = BertModel.from_pretrained(config.bert_path).to(config.device)
    def forward(self,input,mask):
        outputs = self.encoder_1(input, attention_mask=mask, return_dict=True)
        return outputs['pooler_output'], outputs['last_hidden_state']

class Bert_Encoder1(base_model):

    def __init__(self, config):
        super(Bert_Encoder1, self).__init__()

...
from base_function import  train_simple_model,evaluate_strict_model,train_mem_model,select_data,train_mem_model_1
from base_function import generate_current_relation_data,get_proto,generate_relation_data
from base_function import data_augmentation
from model.Graph_Layer import graph_layer
...
def main():
    ...
    for rou in range(config.total_round):
        ...
        while True:
            ...
            state = env.reset()
            prev_act = -1
            prev_state = {"input":None,"teacher":None,"student":None,"T": 0, "T_index": 0, "W": 0, "W_index": 0, "iter": -1}
            states = []
            valid = []
            for steps, (
            training_data, valid_data, test_data, current_relations, historic_test_data, seen_relations) in enumerate(
                ...
                LCT=train_mem_model(config=config, encoder=encoder, drop_layer=droplayer, classifier=classifier,
                                ...
                                new_relation_data=new_relation_data,prev_encoder=prev_encoder,prev_drop_layer=prev_drop_layer,
                                prev_classifier=prev_classifier,state=state,relation_index=relation_index)

                for relation in current_relations:
                    memorized_samples[relation] = select_data(config, encoder, droplayer, training_data[relation])
                ...
                LCD=train_mem_model_1(config=config, encoder=encoder, drop_layer=droplayer, classifier=classifier,
                ...
                prev_drop_layer=deepcopy(droplayer)
                ...
                if prev_state["iter"] != -1:
                    ...
                    RL_Agent.value_q[prev_state["iter"]][prev_act] +=(return_val- RL_Agent.value_q[prev_state["iter"]][prev_act]) / RL_Agent.value_n[prev_state["iter"]][prev_act]
                prev_act = act
                prev_state = state
                state = next_state
                states.append(state)
        ...
        for steps, (
            ...
            cur_acc = evaluate_strict_model(config, encoder, droplayer, classifier,
                                            test_data_1, seen_relations, map_relid2tempid,rel2id)
            total_acc = evaluate_strict_model(config, encoder, droplayer, classifier,
...
if __name__ == '__main__':
    ...
    main()
# forward_acc = evaluate_strict_model(config, encoder, descencoder, graphlayer, classifier, test_data_1,
#                                                 seen_relations, map_relid2tempid,rel2id)
#             forward_accs.append(forward_acc)
#             print(f"task accuracy= {forward_acc}")
#
#             print(f"simple finished")
#             test_cur.append(forward_acc)
#
#         for i in range(8):
#             print(f"the{i}task accuracy= {forward_accs[i]}")
#         result_cur_test.append(np.array(test_cur))
#         avg_result_cur_test = np.average(result_cur_test, 0)
#         print("avg_result_cur_test")
#         print(avg_result_cur_test)
